...
import { ChevronLeft, Sliders, Loader2 } from "lucide-react"
...
export default function EditAttributePage() {
    ...
    const fetchAttribute = async () => {
        try {
            const res = await adminService.getAttributeById(params.id as string)
            if (res.success) {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
                ...
                <p className="text-sm font-black text-gray-400 uppercase tracking-widest">Loading attribute profile...</p>
            </div>
        )
    }
        ...
        <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <h1 className="text-3xl font-black text-gray-900 dark:text-white">Edit Attribute: <span className="text-orange-500">{attribute?.name}</span></h1>
                </div>
            </div>
            ...
            <AttributeForm 
                ...
                isEdit={true} 
                onSuccess={() => router.push("/admin/product-attributes")} 
