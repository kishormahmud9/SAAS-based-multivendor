...
import { Bell, Search, User, Clock, Check, AlertCircle, ShoppingBag, ArrowRight, Settings, Shield, Activity, LogOut } from "lucide-react"
...
const MOCK_NOTIFICATIONS = [
    ...
    { id: 2, title: "System Update", desc: "Version 4.2.1 deployment completed.", time: "1 hour ago", type: "system", icon: Check, color: "text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20" },
    { id: 3, title: "Security Alert", desc: "Multiple failed login attempts detected.", time: "3 hours ago", type: "security", icon: AlertCircle, color: "text-red-600 bg-red-50 dark:bg-red-900/20" },
]
            ...
            <div className="flex items-center justify-between px-8 py-4">
                ...
                <div className="flex-1 max-w-2xl">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-4 py-3 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500"
                        />
                    </div>
                </div>
                ...
                <div className="flex items-center space-x-4 ml-6">
                        ...
                        {isNotificationsOpen && (
                            <div className="absolute right-0 mt-3 w-80 bg-white dark:bg-[#0A0A0B] rounded-[2rem] border border-gray-100 dark:border-gray-800 shadow-2xl animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
                                <div className="p-5 border-b border-gray-50 dark:border-gray-900 flex justify-between items-center">
                                    ...
                                    <span className="text-[10px] font-black text-orange-600 bg-orange-50 dark:bg-orange-900/20 px-2.5 py-1 rounded-full">{MOCK_NOTIFICATIONS.length} New</span>
                                </div>
                    ...
                    <div className="flex items-center space-x-3 pl-4 border-l border-gray-200 dark:border-gray-700 relative" ref={userMenuRef}>
                        ...
                        >
                            <div className="w-10 h-10 bg-gradient-to-br from-blue-600 to-orange-500 dark:from-blue-700 dark:to-orange-600 rounded-xl flex items-center justify-center text-white font-bold shadow-lg group-hover:scale-105 transition-transform">
                                {user?.name?.[0]?.toUpperCase() || "A"}
                            </div>
                            <div className="hidden md:block text-left">
                                <p className="text-sm font-semibold text-gray-900 dark:text-gray-100 group-hover:text-orange-600 transition-colors">{user?.name || "Admin User"}</p>
                                <p className="text-xs text-gray-500 dark:text-gray-400">{user?.email || "admin@readymart.com"}</p>
                            </div>
                        </button>
                        ...
                        {isUserMenuOpen && (
                            <div className="absolute right-0 top-full mt-3 w-64 bg-white dark:bg-[#0A0A0B] rounded-[2rem] border border-gray-100 dark:border-gray-800 shadow-2xl animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
                                <div className="p-6 border-b border-gray-50 dark:border-gray-900 bg-gray-50/50 dark:bg-gray-900/20">
                                    ...
                                    <p className="text-xs font-black text-gray-900 dark:text-white truncate">{user?.email || "admin@readymart.com"}</p>
                                    <div className="mt-3 inline-flex items-center gap-1.5 px-2 py-0.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 rounded-lg text-[9px] font-black uppercase tracking-widest">
